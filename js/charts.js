...
const ChartsManager = {
    ...
    updateGradeDistributionChart() {
        ...
        // Show ALL CGS grades (A1 through NP) so the full scale is visible
    ...
    updateRetentionRateChart(modeOrYear) {
        ...
        const retention = DataStore.getRetentionByProgramme('all');
        const attrition = DataStore.getAttritionByProgramme();
        const programmes = Object.keys(retention).sort();
        ...
        if (isNorm) {
            ...
            chart.data.datasets[0].data = programmes.map(p => {
                const total = retention[p] + attrition[p];
                return total > 0 ? parseFloat((retention[p] / total * 100).toFixed(1)) : 0;
            });
            chart.data.datasets[1].data = programmes.map(p => {
                const total = retention[p] + attrition[p];
                return total > 0 ? parseFloat((attrition[p] / total * 100).toFixed(1)) : 0;
            });
            chart.options.scales.x.max = 100;
            chart.options.scales.x.title.text = 'Percentage (%)';
        } else {
            chart.data.labels = programmes;
            chart.data.datasets[0].data = programmes.map(p => retention[p]);
            chart.data.datasets[1].data = programmes.map(p => attrition[p]);
            chart.options.scales.x.max = 100;
            chart.options.scales.x.title.text = 'Rate (%)';
        }
        chart.update();
    },
    ...
    updateSunburstChart() {
        ...
        const layout = {
            ...
            plot_bgcolor: 'rgba(0,0,0,0)',
            annotations: [{
                text: `<b>All Graduates</b><br>${grandTotal.toLocaleString()}`,
                showarrow: false,
                font: { size: 14, family: 'Inter, sans-serif', color: '#ffffff' },
                x: 0.5, y: 0.5,
    ...
    _buildCoursePassRateFilter() {
        ...
        html += '<option value="bottom10">Bottom 10 (Lowest Pass Rate)</option>';
        ...
        container.querySelector('select').addEventListener('change', (e) => {
            ...
            if (val === 'bottom10') {
                ...
                this._coursePassRateMode = 'bottom10';
            } else {
    ...
    updateCoursePassRateChart(mode) {
        if (mode !== undefined) this._coursePassRateMode = mode;
        const currentMode = this._coursePassRateMode || 'bottom10';
        ...
        let courses;
        if (currentMode === 'single' && this._coursePassRateFilterCourse) {
            courses = allCourses.filter(c => c.course === this._coursePassRateFilterCourse);
        } else {
            courses = allCourses.slice(0, 10);
        }

        // Reset dropdown to bottom10 when toggle buttons are used
        if (mode === 'bottom10' || mode === 'normalized') {
            this._coursePassRateFilterCourse = null;
            const select = document.getElementById('coursePassRateSelect');
            if (select) select.value = 'bottom10';
        }
